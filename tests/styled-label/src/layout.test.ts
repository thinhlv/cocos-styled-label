...
const getSprite = (name: string) => (name === "icon" ? { width: 32, height: 32 } : null);
...
describe("buildLayout — line breaking", () => {
  ...
  test("explicit <br/> creates a new line", () => {
    const segments: ITextSegment[] = [seg("a"), { text: "", style: { isNewLine: true } }, seg("b")];
    const result = buildLayout(makeParams({ segments }));
...
describe("buildLayout — sprites", () => {
  test("sprite segment produces a word with spriteName and correct dimensions", () => {
    ...
    const segments: ITextSegment[] = [{ text: "￼", style: { spriteName: "icon", spriteSize: 20 } }];
    const result = buildLayout(makeParams({ segments }));
    expect(result.lines).toHaveLength(1);
  ...
  test("unknown sprite is ignored (not added to line)", () => {
    const segments: ITextSegment[] = [{ text: "￼", style: { spriteName: "missing" } }];
    const result = buildLayout(makeParams({ segments }));
    expect(result.lines).toHaveLength(0);
  });
  ...
  test("sprite falls back to fontSize when spriteSize is undefined", () => {
    ...
    const segments: ITextSegment[] = [{ text: "￼", style: { spriteName: "icon" } }];
  ...
  test("sprite mixes with text on the same line", () => {
    const segments: ITextSegment[] = [seg("hi"), { text: "￼", style: { spriteName: "icon", spriteSize: 20 } }];
    const result = buildLayout(makeParams({ segments, canvasW: 200 }));
    expect(result.lines).toHaveLength(1);
...
describe("buildLayout — word merging", () => {
  ...
  test("adjacent different-style words are NOT merged", () => {
    const segments: ITextSegment[] = [seg("hello", { color: "#ff0000" }), seg("world")];
    const result = buildLayout(makeParams({ segments }));
    expect(result.lines).toHaveLength(1);
...
describe("buildLayout — sup/sub word vAlign", () => {
  ...
  test('sup word entry preserves vAlign:"top" in style', () => {
    const segments: ITextSegment[] = [{ text: "$", style: { vAlign: "top", size: 20 } }, { text: "2,000" }];
  ...
  test('sub word entry preserves vAlign:"bottom" in style', () => {
    const segments: ITextSegment[] = [{ text: "x", style: { vAlign: "bottom", size: 20 } }];
...
describe("wordBaselineY", () => {
  ...
  test("vAlign=undefined: same as baseline", () => {
    expect(wordBaselineY(lineTop, lineH, wordAscent, lineAscent, wordH, undefined)).toBe(
      wordBaselineY(lineTop, lineH, wordAscent, lineAscent, wordH, "baseline"),
    );
  });
...
describe("[BUG] space after <sprite> tag — text must not touch sprite", () => {
  ...
  const getSpr = (name: string) => (name === "icon" || name === "0" ? { width: 32, height: 32 } : null);
