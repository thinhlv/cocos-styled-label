...
function makeMockRD() {
    ...
    const data: any[] = Array.from({ length: maxV }, () => ({
        x: 0, y: 0, z: 0, u: 0, v: 0, color: {},
    }));

    const rd: any = {
        _frame: null as any,
        get frame() { return rd._frame; },
        set frame(v: any) { rd._frame = v; },
        textureDirty: true,
        passDirty: true,
        dataHash: 0,
        vertDirty: false,
        _vc: 0,
        get vertexCount() { return rd._vc; },
        get dataLength() { return data.length; },
        ...
        resize: jest.fn((vc: number) => { rd._vc = vc; }),
